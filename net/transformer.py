...
class TransformerEncoderLayer(torch.nn.Module):
    ...
    def forward(self, src, src_mask=None, src_key_padding_mask=None):
        # Query embeddings of shape :math:(L目标序列长度, N批处理大小, E_qv嵌入维度) when batch_first=False or :math:(N, L, E_q) when batch_first=True
        # src_key_padding_mask shape (N批处理大小, S源序列长度) [batch_size, seq_len]
